...
void SelectStoppingMuons_EastCryo(std::string iFile_, std::string oFile_, std::string oFile2_, float lifetime_ = 7.0) {
  
  // open file and read tree
  TFile *file = TFile::Open(iFile_.c_str(), "READ");
  TTreeReader treeW("t0TreeStoreE/TimedTrackStorage", file);
  ...
  // parameters
  const int offset = 10;
  const float yh = 134.96, yl = -181.86, xl = -358.73, xh = -61.7, zh = 894.951, zl = -894.951;
